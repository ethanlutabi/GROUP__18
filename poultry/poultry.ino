    ...
    if (scale.is_ready()) {
      ...
      // Weight above average: Open the second door, turn off first LED, and turn on second LED
        ...
        if (!secondDoorOpen || weightChanged) {
          ...
          // Increment second door open counter
          secondDoorOpenCount++;
      ...
      // Display door open counts on the same line
      lcd.setCursor(0, 1);
      lcd.print("BW: ");
      lcd.print(firstDoorOpenCount);
      lcd.print(" AW: ");
      lcd.print(secondDoorOpenCount);

      previousWeight = weight; // Update previous weight with the current weight
